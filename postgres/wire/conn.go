...
func defaultTag(plan ir.Node) string {
	switch plan.(type) {
	case *ir.Project, *ir.Scan, *ir.Values, *ir.Filter, *ir.Sort, *ir.Limit, *ir.Aggregate, *ir.Distinct, *ir.Union, *ir.SubqueryAlias, *ir.Window, *ir.Unnest, *ir.GenerateSeries, *ir.Recursive, *ir.RecursiveRef:
		return "SELECT"
	case *ir.CreateTable:
		return "CREATE TABLE"
	case *ir.DropTable:
